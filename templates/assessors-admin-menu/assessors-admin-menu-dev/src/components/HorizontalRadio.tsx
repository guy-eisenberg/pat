...
const HorizontalRadio: React.FC<HorizontalRadioProps> = ({
  ...
  return (
    <div
      {...rest}
      className={c(
        'h-16 rounded-full border border-theme-border bg-[#fbfbfb] px-6 py-2 text-theme-medium-gray',
        rest.className
      )}
      ref={containerRef}
    >
      <div className="relative flex h-full justify-between">
        <div
          className={c(
            'absolute top-0 box-content h-full -translate-x-4 rounded-full bg-theme-blue px-4 transition-all'
          )}
